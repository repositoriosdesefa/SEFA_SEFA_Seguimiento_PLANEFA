...
CARPETA <- file.path(PROYECTO, "Oficios_OCI")
# Parámetros No Locales (En la web)
NOMBRE_PEDIDO <- "RT2 - OCI - "
EXHORTACION_RT <- ""
TABLA_INSUMOS <- "INSUMOS_RT2_OCI"
...
RT_2_OCI_INSUMOS <- read_sheet(ss = EXHORTACION_RT,
                               sheet = TABLA_INSUMOS)
...
# ii) Email: Cabecera
Arriba <- add_image(
  file = "https://i.imgur.com/niEfPhj.png",
  width = 1000,
  align = c("right"))
Cabecera <- md(Arriba)
...
# iu) Botón CODE y SC
boton_SC <- add_cta_button(
  url = "",
  text = "Oficios a OCI - PLANEFA"
)
...
INSUMOS <- RT_2_OCI_INSUMOS %>%
  filter(TIPO_EXHORTACION=="Tipo 1" | TIPO_EXHORTACION=="Tipo 2")  %>%
  arrange(OD_COMPETENTE, EFA_5)

SEDE_CENTRAL <- INSUMOS %>%
  filter(OD_COMPETENTE == "Sede Central del OEFA")
...
correo_OD <- function(od, jefe,
  ...
  Asunto_OD <- paste("Oficios Automatizados PLANEFA | Oficios a OCI RT2")
...
seguimiento PLANEFA: ", num_oficios_n, " oficios a OCI en este segundo trimestre.
 ...
 **Es importante tomar en cuenta las siguientes indicaciones:**
 - Les recordamos que los plazos de envío de los oficios están establecidos en el cronograma al que
   pueden acceder en el siguiente enlace: https://docs.google.com/spreadsheets/d/1WDiAxe6uyIF60nzuoWk9_i6PXhWu2blOqz68bmcbdWg/edit?usp=sharing
...
CONTACTO_OD <- CONTACTO_OD %>%
  ...
  arrange(NUMERO_OFICIOS)

pwalk(list(CONTACTO_OD$ID_OD_COMPLETO,
           CONTACTO_OD$`NOMBRE JEFE DE OD`,
           CONTACTO_OD$CORREO_OD_CONTACTO_PLANEFA,
           CONTACTO_OD$CORREO_RECORDATORIO_JEFE,
           CONTACTO_OD$NUMERO_OFICIOS,
           CONTACTO_OD$URL_CARPETA),
      slowly(R_correo_OD, 
             rate_backoff(10, max_times = Inf)))

# IV.2 Correo a Sede Central ----
Asunto_SC <- paste("Oficios Automatizados PLANEFA | Seguimiento al reporte trimestral")
num_oficios_sc <- paste0("**",nrow(SEDE_CENTRAL),"**")

# Cuerpo del mensaje
Cuerpo_del_mensaje_SC <- blocks(
  md(c("
Buenas tardes, 

**Jefa de Supervision a EFA - SEFA**

El Equipo de Proyectos e Innovación de la Subdirección de Seguimiento de Entidades de Fiscalización
Ambiental le informa que se han generado exitosamente los documentos automatizados para el seguimiento 
PLANEFA: ", num_oficios_sc, " oficios a OCI en este segundo trimestre.
...
email_SC <- compose_email(
  header = Cabecera,
  body = Cuerpo_del_mensaje_SC, 
  footer = Pie_de_pagina,
  content_width = 1000
)
...
num_oficios_code <- paste0("**",nrow(INSUMOS)-nrow(SEDE_CENTRAL),"**")

# Cuerpo del mensaje
Cuerpo_del_mensaje_CODE <- blocks(
  md(c("
Buenas tardes, Alicia Rodriguez Peralta:

**Coordinadora de Oficinas Desconcentradas - CODE**

El Equipo de Proyectos e Innovación de la Subdirección de Seguimiento de Entidades de Fiscalización
Ambiental le informa que se han generado exitosamente los documentos automatizados para el seguimiento 
PLANEFA: ", num_oficios_code ," oficios a OCI en este segundo trimestre.
